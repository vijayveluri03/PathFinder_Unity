...
public class WaypointManagerEditor : Editor 
{
        ...
        for (int i = 0; i < script.pathList.Count; i++)
        {
            ...
            if (CustomGUI.HeaderButton(script.pathList[i].pathName, null, delAction))
            {
                ...
                for (int j = 0; j < wayPointList.Count; j++)
                {
                    GUILayout.BeginHorizontal();
                    {
                        wayPointList[j].position = EditorGUILayout.Vector3Field("Point " + wayPointList[j].autoGeneratedIndex, wayPointList[j].position);
    ...
    public void DrawGUIDisplayForPoints()
    {
        ...
        for ( int i = 0; i < script.selected.points.Count; i++ ) 
        {
            ...
            GUI.Box( new Rect (guiPosition.x - 10, guiPosition.y - 30, 20, 20 ), currPoint.autoGeneratedIndex.ToString());
        }
        Handles.EndGUI();
    }
    ...
    void ReIndexPoints ( )
    {
        ...
        for ( int i = 0; i < script.selected.points.Count; i++  )
        {
            script.selected.points[i].autoGeneratedIndex = i;
        }
    }
    void SetLinePoints()
    {
