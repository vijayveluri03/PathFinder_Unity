...
namespace QPathFinder
{
	...
	{
		[Obsolete("Use FindNearestNode instead. This will be removed in the next update", true)]
		Simple = 0,		// Go to nearest Node, Stop there 
		[Obsolete("Use FindNearestPointOnTheConnection instead. These will be removed in the next update", true)]
		Intermediate = 1,	// go to the nearest point on the connections ( between the nodes ) 
		[Obsolete("Use FindPathBeyondTheNodesAndConnections instead. These will be removed in the next update", true)]
		Complex = 2,			// go anywhere beyond the nodes and connections 

		// Same as the above ones
		FindNearestNode = 0,							// Same as Simple. Finds the path till the nearest NODE
		FindNearestPointOnTheConnection = 1,			// Same as Intermediate. Finds path till the nearest connection ( between two nodes ). this will be much closer than what we get with 'FindNearestNode'
		FindPathBeyondTheNodesAndConnections = 2		// Same as Complex. Finds path beyond the nodes and connections. The path can deviate from the nodes and connections. 
	}
	...
	public static class PathFinderExtensions
	{
		...
		/// <param name="searchMode"> The search path which you want to take. </param>
		/// <param name="OnPathFound">Callback once the path is found</param>
    ...
    public static class PathFollowerUtility 
    {
		...
		{
			int nearestNodeFromStart = manager.FindNearestNode ( startPoint );
			int nearestNodeFromEnd = -1;
			if ( nearestNodeFromStart != -1 )
				nearestNodeFromEnd = manager.FindNearestNode ( endPoint );
			
			if ( QPathFinder.Logger.CanLogInfo ) QPathFinder.Logger.LogInfo("Nearest point from start" + startPoint + " is " + nearestNodeFromStart, true );
			if ( QPathFinder.Logger.CanLogInfo ) QPathFinder.Logger.LogInfo("Nearest point from end:" + endPoint + " is " + nearestNodeFromEnd, true );

			if ( nearestNodeFromEnd == -1 || nearestNodeFromStart == -1 )
			{
				if ( QPathFinder.Logger.CanLogError ) QPathFinder.Logger.LogError("Could not find path between " + nearestNodeFromStart + " and " + nearestNodeFromEnd, true );
				OnPathFound( null );
				return;
			}
			...
			{ 
				...
				if ( searchMode == SearchMode.FindNearestPointOnTheConnection || searchMode == SearchMode.FindPathBeyondTheNodesAndConnections )
				{
					if ( allNodes.Count > 1 ) 
					{
						...
						// Finding the nearest point of entry
						{
						...
						// Finding the nearest point of exit 
						{
							shortestPathID = -1;
				...
				
				{
					...
					// Go beyond the Nodes and Connections
					if(searchMode == SearchMode.FindPathBeyondTheNodesAndConnections)
			...
			manager.FindShortestPathOfNodes ( nearestNodeFromStart, nearestNodeFromEnd, execution, onPathOfNodesFound );
		}
